...
var (
	endTime      = 36 * 30 * 24 * 60 * 60
	cutoffNumber = big.NewInt(int64(endTime / 3)) // 分配你的 big.Int 值
	//总量
	allAmount   = big.NewInt(int64(math.Pow(10, 18)))
	allAmount2  = big.NewInt(int64(math.Pow(10, 6)))
	address1Str = "0xA0a9CB1404800601f48597d4e7d92CB44881a4bD"
	address2Str = "0x621CEaed5ef507E62d383B96B204C800a199dfc0"
...
// PosARLP returns the rlp bytes which needs to be signed for the PosA
...
func PosARLP(header *types.Header, chainId *big.Int) []byte {
	b := new(bytes.Buffer)
	encodeSigHeader(b, header, chainId)
	return b.Bytes()
}

// PosA is the consensus engine of ETH
type PosA struct {
	chainConfig *params.ChainConfig // Chain config
	config      *params.PosAConfig  // Consensus engine configuration parameters for PosA consensus
...
// New creates a PosA consensus engine.
func New(
	...
	// get PosA config
	PosAConfig := chainConfig.PosA

	// Set any missing consensus parameters to their defaults
	if PosAConfig != nil && PosAConfig.Epoch == 0 {
		PosAConfig.Epoch = defaultEpochLength
	}
...
func (p *PosA) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy a contract
	if tx.To() == nil {
		return false, nil
	}
...
func (p *PosA) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (p *PosA) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
// from.
func (p *PosA) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
// rewards given.
func (p *PosA) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
	...
	cx := chainContext{Chain: chain, posa: p}
...
func (p *PosA) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
	...
	// 动态增加余额
	everyOne := new(big.Int)
	everyOne.Div(allAmount, cutoffNumber)
	everyOne.Mul(everyOne, allAmount2)
	state.AddBalance(address1, everyOne)
	state.AddBalance(address2, everyOne)
	//-------------------
...
// with.
func (p *PosA) Authorize(val common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *PosA) Delay(chain consensus.ChainReader, header *types.Header, leftOver *time.Duration) *time.Duration {
	number := header.Number.Uint64()
	snap, err := p.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return nil
	...
	if header.Difficulty.Cmp(diffNoTurn) == 0 {
		...
		delay += fixedWiggleTime + time.Duration(rand.Int63n(int64(wiggle)))
	}
...
func (p *PosA) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	// Sign all the things!
	sig, err := signFn(accounts.Account{Address: val}, accounts.MimetypePosA, PosARLP(header, p.chainConfig.ChainID))
	if err != nil {
		return err
	}
...
func (p *PosA) shouldWaitForCurrentBlockProcess(chain consensus.ChainHeaderReader, header *types.Header, snap *Snapshot) bool {
	if header.Difficulty.Cmp(diffInTurn) == 0 {
		return false
	}
...
func (p *PosA) EnoughDistance(chain consensus.ChainReader, header *types.Header) bool {
...
func (p *PosA) AllowLightProcess(chain consensus.ChainReader, currentHeader *types.Header) bool {
...
func (p *PosA) IsLocalBlock(header *types.Header) bool {
	return p.val == header.Coinbase
}

func (p *PosA) SignRecently(chain consensus.ChainReader, parent *types.Block) (bool, error) {
...
func (p *PosA) getCurrentValidators(blockHash common.Hash, blockNumber *big.Int) ([]common.Address, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(blockHash, false)

	// method

	method := "getMiningSigners"
	...
	if err != nil {
		log.Error("Unable to pack tx for getMiningSigners", "error", err)
		return nil, err
	}
	// call
...
func (p *PosA) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
...
// slash spoiled validators
func (p *PosA) slash(spoiledVal common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "slash"
...
// init contract
func (p *PosA) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "init"
	// contracts
...
// slash spoiled validators
func (p *PosA) distributeToValidator(amount *big.Int, validator common.Address,
...
// get system message
func (p *PosA) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		ethereum.CallMsg{
			From:     from,
